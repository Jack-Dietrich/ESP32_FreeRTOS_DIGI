...
// Pins
static const int adc_pin = 34;//changed so we read from a pin that has info
...
static volatile uint16_t buf_0[BUF_LEN];      // One buffer in the pair of the double buffer
static volatile uint16_t buf_1[BUF_LEN];      // The other buffer in the pair
static volatile uint16_t* write_to = buf_0;   // Double buffer write pointer, starts at start of buffer
...
static float adc_avg;                         // Just for calculating the average of the last values from the adc
